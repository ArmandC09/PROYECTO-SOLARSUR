          ...
          <div className="ss-overlay" onClick={e => { if(e.target===e.currentTarget) setEdOpen(false) }}>
            ...
            <div className="ss-modal-foot">
              ...
              <button
                ...
                title={edClient?.phone ? `Enviar al WhatsApp de ${edClient.name}` : 'El cliente no tiene teléfono registrado'}
                style={{
                  display: 'flex', alignItems: 'center', gap: 7,
                  padding: '10px 20px', borderRadius: 10, border: 'none',
                  background: edClient?.phone ? 'linear-gradient(135deg,#25d366,#128c7e)' : '#d1d5db',
                  color: '#fff', fontWeight: 700, fontSize: 14,
                  cursor: edClient?.phone ? 'pointer' : 'not-allowed',
                  boxShadow: edClient?.phone ? '0 4px 14px rgba(37,211,102,0.35)' : 'none',
                  opacity: edClient?.phone ? 1 : 0.6
                }}
              >
