      ...
      {edOpen && (
        <ModalPortal>
          <div className="ss-overlay" onClick={e => { if(e.target===e.currentTarget) setEdOpen(false) }}>
            ...
            <div className="ss-modal-foot" style={{gap:8}}>
              <button type="button" className="ss-btn-cancel" onClick={()=>setEdOpen(false)}>Cerrar</button>
              <button
                type="button"
                onClick={sendWhatsApp}
                title={edClient?.phone ? `Abrir WhatsApp de ${edClient?.name}` : 'El cliente no tiene teléfono registrado'}
                style={{
